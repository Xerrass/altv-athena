...
import { XVEHICLE_EVENTS, XVEHICLE_VIEW_EVENTS } from '../shared/enum/events';
...
export class VehicleFunctions {
    ...
    static handleEnteredVehicle(vehicle: alt.Vehicle, seat: number) {
        ...
        if (seat == 1) {
            ...
            alt.emit(XVEHICLE_VIEW_EVENTS.OPEN, vehicle)
        ...
        if (seat == 1) {
            if (isVehicleType(data.type, VEHICLE_TYPE.CAR) || isVehicleType(data.type, VEHICLE_TYPE.AMPHIBIOUS_AUTOMOBILE) || isVehicleType(data.type, VEHICLE_TYPE.QUADBIKE) || isVehicleType(data.type, VEHICLE_TYPE.AMPHIBIOUS_QUADBIKE)) {
    ...
    static handleLeftVehicle(vehicle: alt.Vehicle, seat: number) {
        ...
        if (seat == 1) {
            ...
            alt.emit(XVEHICLE_VIEW_EVENTS.CLOSE)
            if (engineInterval) {
